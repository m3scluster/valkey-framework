...
type Task = { ID: string; Role: string; State: string; Host?: string; Port?: number; CPU?: number; Memory?: number; Disk?: number; Updated?: string };
...
type TaskMetric = { allocated_cpu: number; allocated_memory_mb: number; allocated_disk_mb: number; usage_available: boolean; usage_error?: string; usage_cpu_cores?: number; usage_memory_mb?: number; usage_disk_mb?: number };
type Metrics = { framework_id: string; scheduler_connected: boolean; desired: boolean; total: number; running: number; staging: number; failed: number; nodes?: Record<string, TaskMetric>; valkey?: { available: boolean; error: string; sections: Record<string, Record<string, string | number>> } };
...
function App() {
  ...
  return <div className="shell" data-theme={theme}>
    ...
    <main>
      ...
      {view === 'nodes' && <><section className="nodes-head"><div><span className="label">INSTANCES</span><h3>Valkey Nodes</h3></div><span className="refresh">Refreshes every 5 seconds</span></section><div className="node-grid">{liveTasks.length ? liveTasks.map(task => { const metric = metrics?.nodes?.[task.ID]; const cpu = metric?.usage_available ? metric.usage_cpu_cores : metric?.allocated_cpu ?? task.CPU; const memory = metric?.usage_available ? metric.usage_memory_mb : metric?.allocated_memory_mb ?? task.Memory; const disk = metric?.usage_available ? metric.usage_disk_mb : metric?.allocated_disk_mb ?? task.Disk; return <article className="node card" key={task.ID}><div className="node-top"><ValkeyClusterMark /><span className={`badge ${task.State.toLowerCase().replace('task_', '')}`}>{task.State.replace('TASK_', '')}</span></div><h3>{task.Role}</h3><p>{task.Host || 'Mesos Agent'}</p><div className="node-data"><span>PORT <b>{task.Port ?? '—'}</b></span><span>UPDATED <b>{task.Updated ? new Date(task.Updated).toLocaleTimeString('en-US') : '—'}</b></span></div><div className="resource-grid"><span>CPU <b>{cpu != null ? `${cpu.toFixed(2)} cores` : '—'}</b></span><span>MEM <b>{memory != null ? `${memory.toFixed(1)} MB` : '—'}</b></span><span>DISK <b>{disk != null ? `${disk.toFixed(1)} MB` : '—'}</b></span></div>{metric?.usage_available ? <small className="usage-available">Live usage from Mesos agent</small> : <small className="usage-unavailable">Usage unavailable from Mesos agent</small>}</article> }) : <div className="empty card">No live nodes. The scheduler is waiting for Mesos Offers.</div>}</div></>}
