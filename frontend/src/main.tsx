...
function App() {
  ...
  return <div className="shell" data-theme={theme}>
    ...
    <main>
      ...
      {view === 'nodes' && <><section className="nodes-head"><div><span className="label">INSTANCES</span><h3>Valkey Nodes</h3></div><span className="refresh">Refreshes every 5 seconds</span></section><div className="node-grid">{liveTasks.length ? liveTasks.map(task => <article className="node card" key={task.ID}><div className="node-top"><ValkeyClusterMark /><span className={`badge ${task.State.toLowerCase().replace('task_', '')}`}>{task.State.replace('TASK_', '')}</span></div><h3>{task.Role}</h3><p>{task.Host || 'Mesos Agent'}</p><div className="node-data"><span>PORT <b>{task.Port ?? '—'}</b></span><span>UPDATED <b>{task.Updated ? new Date(task.Updated).toLocaleTimeString('en-US') : '—'}</b></span></div><div className="resource-grid"><span>CPU <b>{task.CPU ? `${task.CPU} cores` : '—'}</b></span><span>MEM <b>{task.Memory ? `${task.Memory} MB` : '—'}</b></span><span>DISK <b>{task.Disk ? `${task.Disk} MB` : '—'}</b></span></div><small className="usage-unavailable">Usage unavailable from Mesos TaskStatus</small></article>) : <div className="empty card">No live nodes. The scheduler is waiting for Mesos Offers.</div>}</div></>}
